...
import {
    deleteOrderByOrderIdUsingDelete,
    getUserOrdersByUserIdUsingGet,
    getOrderDetailByOrderIdUsingGet,
} from '@/api/order.api';
...
const OrdersPage = () => {
    ...
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const response = await getUserOrdersByUserIdUsingGet(userId);
            const filtered = statusFilter ? response.filter((o: OrderResponse) => o.status === statusFilter) : response;
            setOrders(filtered);
        } catch (error) {
    ...
    const cancelOrder = async (orderId: number) => {
        try {
            await deleteOrderByOrderIdUsingDelete(orderId);
    ...
    const showOrderDetail = async (order: OrderResponse) => {
        try {
            const itemsResponse = await getOrderDetailByOrderIdUsingGet(order.id);
            setSelectedOrder({ ...order, orderItems: itemsResponse });
                            ...
                            >
                                <div>
                                    ...
                                    {order.address && (
                                        <>
                                            <p>
                                                收貨地址: {order.address.city}
                                                {order.address.district}
                                                {order.address.detailAddress}
                                            </p>
                                            <p>
                                                收貨人: {order.address.recipientName} ({order.address.phone})
                                            </p>
                                        </>
                                    )}
                                </div>
